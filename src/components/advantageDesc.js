...
import './advantageDesc.less';

class AdvantageDesc extends PureComponent{
    render(){
        const {title, p, color, to} = this.props;
        return (
            <div className="advantageDesc-component" style={{border: `2px solid ${color}`}}>
                <div className="triangle-container" style={to === 'right' ? {right: '-16px'} : {left: '-16px'}}>
                    <Triangle size={16} outerColor={color} innerColor='whitesmoke' to={to}></Triangle>
                </div>
